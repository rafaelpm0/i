...
public class Main {
    public static void main(String[] args) {
        ...
        try {
            ...
            for (Filme filme : filmes) {
                ...
                filme.score = scoreFinal;

                System.out.printf(
                    "Filme: %-35s | Nota: %.1f | Votos: %.0f | Pop: %.2f | Orc: %.0f | Score: %.2f%n",
                    titulo, nota, votos, popularidade, orcamento, scoreFinal
                );
            }

            filmes.sort((a, b) -> Float.compare(b.score, a.score));

            System.out.println("\nTOP 20 FILMES:\n");

            for (int i = 0; i < Math.min(20, filmes.size()); i++) {
                Filme f = filmes.get(i);

                System.out.printf(
                    "%dº %-35s | Score: %.2f | Nota: %.1f | Votos: %.0f%n",
                    i + 1,
                    f.title,
                    f.score,
                    f.vote_average,
    ...
    private static void aplicarRegraE(HashMap<String, Float> pertinencias,
                                      String var1,
                                      String var2,
                                      String resultado,
                                      HashMap<String, Float> conclusoes) {
    ...
    private static void aplicarRegras(HashMap<String, Float> pertinencias,
                                      HashMap<String, Float> conclusoes) {
